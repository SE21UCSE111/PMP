...
function ProfessorHomePage() {
  ...
  const handleAcceptTeam = (teamId, projectId) => {
    ...
    // Only add the team to acceptedTeams if it is not already accepted
        ...
        if (project.id === projectId) {
          return {
            ...
            teams: project.teams.map((team) =>
              team.id === teamId ? { ...team, hideButtons: true, status: 'accepted' } : team
            ),
          };
        }
        return project;
      ...
      try {
        ...
        if (data.status === "success") {
          ...
          const updatedProjects = data.projects.map((project) => ({
            ...project,
            teams: project.teams.map((team) => {
              const isAccepted = acceptedTeams.some(
                (acceptedTeam) =>
                  acceptedTeam.teamId === team.id && acceptedTeam.projectId === project.id
              );
              return {
                ...team,
                hideButtons: isAccepted, // Hide buttons if the team is accepted
                status: team.status || (isAccepted ? 'accepted' : 'pending'), // Use status from the API or fallback to pending
              };
            }),
          }));
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <button onClick={() => navigate("/project_creation")} style={{ padding: "10px", width: "220px" }}>
        My Projects
      </button>
            ...
            >
              ...
              {project.teams.map((team, teamIndex) => (
                ...
                >
                  ...
                  <div>
                    {/* Show buttons only if the team status is pending */}
                    {team.status === 'pending' && !team.hideButtons && (
                      <>
                        <button
                    ...
                    {team.status === 'accepted' && (
                      <p style={{ color: 'green' }}>Team Accepted</p>
                    )}
                  </div>
                </div>
              ))}
